...
. Before using a FUSE file system we need to load the man:fusefs[5] kernel module:
+
[source, shell]
....
# kldload fusefs
...
HAST provides synchronous block-level replication between two physical machines: the _primary_ node and the _secondary_ node. These two machines together are referred to as a cluster.
