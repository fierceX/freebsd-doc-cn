...
* 了解 UNIX(R) 和 crossref:basics[基础知识, FreeBSD 基础知识] 。
* 熟悉 crossref:kernelconfig 的基础知识 [kernelconfig ，内核配置和编译] 。
* 在 FreeBSD 中，跨引用端口（ crossref:ports[ports , installing software] ）会让您感到舒适。
* 对 FreeBSD 中的 crossref:disks[disks , disks] 、存储和设备名称有一些了解。
...
=== ext2 / ext3 / ext4 是一系列的文件系统类型，用于在 Linux 操作系统中管理磁盘上的文件和目录。这些文件系统类型是为了提供更高级的功能和性能而逐步发展的。 ext2 是最早的版本，提供了基本的文件系统功能。 ext3 在 ext2 的基础上增加了日志功能，可以更好地恢复文件系统的一致性。 ext4 是 ext3 的进一步改进，提供了更高的性能和更大的文件系统容量支持。这些文件系统类型在 Linux 系统中被广泛使用。
