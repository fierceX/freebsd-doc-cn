...
* message/rfc822 是一种 Internet 标准格式，用于表示电子邮件消息。它定义了邮件消息的结构和内容，包括发件人、收件人、主题、日期、正文和附件等信息。这种格式通常用于电子邮件的传输和存储。
* 多部分 / 可选
* 多部分 / 相关
* 多部分 / 签名
* 文本 /HTML
* asciidoc 是一种轻量级的标记语言，用于编写技术文档和文档生成工具。它使用简单的文本格式来描述文档的结构和样式，并支持各种功能，如标题、段落、列表、链接、图像和表格。 asciidoc 还可以通过使用内联代码、代码块和语法高亮来编写和显示代码示例。它还支持生成多种输出格式，如 HTML 、 PDF 和 EPUB 。 asciidoc 易于学习和使用，是许多技术写作和文档团队的首选工具。
* 文本 / x- 差异
* 文本 /x-patch
...
=== X Window System （简称 X11 或 X ）是一个用于图形用户界面（ GUI ）的开放式网络协议和软件系统。它提供了一个框架，使得应用程序可以在不同的计算机上显示图形和接收用户输入。 X Window System 是在 1984 年由麻省理工学院（ MIT ）开发的，现在已经成为了许多操作系统的标准图形界面。它使用客户端 - 服务器模型，其中 X 服务器运行在计算机上，负责处理图形和输入，而 X 客户端运行在用户的计算机上，通过网络连接到 X 服务器。 X Window System 支持多窗口、多任务和多用户，并且可以在不同的硬件平台上运行。它还提供了丰富的图形库和工具，使得开发人员可以轻松地创建和定制图形界面应用程序。
