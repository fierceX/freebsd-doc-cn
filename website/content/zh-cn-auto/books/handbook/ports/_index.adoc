...
FreeBSD 的“端口”是一组文件，旨在自动化从源代码编译应用程序的过程。组成端口的文件包含了自动下载、提取、打补丁、编译和安装应用程序所需的所有必要信息。
...
本章的其余部分将解释如何使用软件包和端口在 FreeBSD 上安装和管理第三方软件。
...
* FreeBSD 网站维护着一个最新的可搜索的应用程序列表，位于链接： https://www.FreeBSD.org/ports/[Ports Portal] 。可以通过应用程序名称或软件类别来搜索端口。
* Dan Langille 维护着 https://www.freshports.org/[FreshPorts] ，该网站提供了一个全面的搜索工具，并跟踪 Ports Collection 中应用程序的变化。注册用户可以创建一个定制的监视列表，以便在所监视的端口更新时收到自动邮件通知。
...
`Quarterly` 分支为用户提供了更可预测和稳定的端口和软件包安装和升级体验。这主要通过只允许非功能性更新来实现。季度分支旨在接收安全修复（可能是版本更新或提交的回溯）、错误修复和端口合规性或框架更改。季度分支在每年的一月、四月、七月和十月的季度初从 HEAD 中切出。分支的命名方式根据它们创建的年份（ YYYY ）和季度（ Q1-4 ）。例如， 2023 年 1 月创建的季度分支被命名为 2023Q1 。而 `Latest` 分支为用户提供了软件包的最新版本。
...
该文件采用 UCL 格式。有关 man:libucl[3] 语法的更多信息，请访问链接： https://github.com/vstakhov/libucl[official UCL website] 。
...
Ports Collection 是一组 `Makefiles` 、补丁和描述文件。每组这些文件用于在 FreeBSD 上编译和安装单个应用程序，被称为一个_端口_。

默认情况下， Ports Collection 本身存储在 `/usr/ports` 的子目录中。

[WARNING]
====
在安装和使用 Ports Collection 之前，请注意通常不建议使用 Ports Collection 与通过 pkg 提供的二进制软件包一起安装软件。 pkg 默认跟踪 ports 树的季度分支发布，而不是 HEAD 。与季度分支发布中的对应端口相比， HEAD 中的端口的依赖关系可能不同，这可能导致 pkg 安装的依赖关系与 Ports Collection 中的依赖关系发生冲突。如果必须同时使用 Ports Collection 和 pkg ，请确保您的 Ports Collection 和 pkg 位于相同的 ports 树分支发布上。
====

Ports 集合包含了软件类别的目录。每个类别中都有针对个别应用程序的子目录。每个应用程序子目录包含一组文件，告诉 FreeBSD 如何编译和安装该程序，称为_ports skeleton_。每个端口骨架包括以下文件和目录：

* *Makefile* ：包含指定应用程序如何编译以及其组件应安装在何处的语句。
* *distinfo* ：包含构建端口所需下载的文件的名称和校验和。
* *files/* ：这个目录包含了在 FreeBSD 上编译和安装程序所需的补丁文件。这个目录也可能包含其他用于构建端口的文件。
* *pkg-descr* ：提供程序的更详细描述。
* *pkg-plist* ：一个包含所有将由端口安装的文件的列表。它还告诉端口系统在卸载时要删除哪些文件。

一些端口包括 `pkg-message` 或其他文件来处理特殊情况。有关这些文件以及端口的更多详细信息，请参考 extref:{porters-handbook}[FreeBSD Porter's Handbook] 。
...
在使用端口编译应用程序之前，必须先安装端口集合。如果在安装 FreeBSD 时没有安装它，请使用以下方法之一进行安装：
...
如果需要对端口树进行更多的控制，或者需要维护本地更改，或者正在运行 FreeBSD-CURRENT ，可以使用 Git 来获取 Ports Collection 。有关 Git 的详细描述，请参阅 extref:{committers-guide}[Git Primer , git-primer] 。

. 在使用 Git 检出端口树之前，必须先安装 Git 。如果已经存在一个端口树的副本，请按照以下方式安装 Git ：
+
[source, shell]
....
...
....
+
如果端口树不可用，或者正在使用 pkg 来管理软件包，可以将 Git 作为一个软件包安装：
+
[source, shell]
....
# pkg install git
...
在编译任何端口之前，请确保按照前一节中的描述更新 Ports Collection 。由于安装任何第三方软件都可能引入安全漏洞，建议首先在 https://vuxml.freebsd.org/[] 上检查与该端口相关的已知安全问题。或者，在安装新端口之前运行 `pkg audit -F` 。此命令可以配置为在每日安全系统检查期间自动执行安全审计和漏洞数据库的更新。有关更多信息，请参阅 man:pkg-audit[8] 和 man:periodic[8] 。
====
...
在构建端口后，有几种方法可以重新访问端口的构建选项菜单，以添加、删除或更改这些选项。一种方法是进入包含端口的目录，然后输入 `make config` 命令。另一种选择是使用 `make showconfig` 命令。还可以执行 `make rmconfig` 命令，该命令将删除所有已选择的选项，并允许您重新开始。所有这些选项以及其他选项都在 man:ports[7] 中详细解释。

端口系统使用 man:fetch[1] 来下载源文件，该工具支持各种环境变量。如果 FreeBSD 系统位于防火墙或 FTP/HTTP 代理后面，可能需要设置 `FTP_PASSIVE_MODE` 、 `FTP_PROXY` 和 `FTP_PASSWORD` 变量。有关支持的变量的完整列表，请参阅 man:fetch[3] 。

对于不能始终连接到互联网的用户，可以在 `/usr/ports` 目录下运行 `make fetch` 命令，以获取所有的 distfiles ，或者在某个分类目录（例如 `/usr/ports/net` ）或特定的端口骨架中运行。请注意，如果一个端口有任何依赖项，那么在分类目录或端口骨架中运行此命令将不会获取来自其他分类的端口的 distfiles 。相反，使用 `make fetch-recursive` 命令也可以获取端口的所有依赖项的 distfiles 。
...
使用 `pkg delete` 命令可以卸载已安装的端口。有关使用此命令的示例，请参阅 man:pkg-delete[8] 手册页。
...
package:ports-mgmt/portmaster[] 是一个非常小的工具，用于升级已安装的端口。它旨在使用安装在 FreeBSD 基本系统中的工具，而不依赖其他端口或数据库。要将此实用程序安装为一个端口：
...
[NOTE]
====
默认情况下， Portmaster 在删除现有端口之前会创建一个备份包。如果新版本的安装成功， Portmaster 会删除备份。使用 `-b` 指令可以让 Portmaster 不自动删除备份。添加 `-i` 可以启动 Portmaster 的交互模式，在升级每个端口之前提示确认。还有许多其他选项可用。请阅读 man:portmaster[8] 的手册页面，了解有关它们使用的详细信息。
====
...
Portmaster 还可以用于在系统上安装新的端口，在构建和安装新的端口之前升级所有依赖项。要使用此功能，请指定 Ports Collection 中端口的位置：
...
package:ports-mgmt/portupgrade[] 是另一个可以用来升级端口的实用工具。它安装了一套可以用来管理端口的应用程序。然而，它依赖于 Ruby 。要安装该端口：
...
如果指定了 `-P` ，只需获取端口 distfiles 或软件包，而不需要构建或安装任何内容，请使用 `-F` 。有关所有可用开关的更多信息，请参阅 `portupgrade` 的手册页。
...
使用 Ports Collection 会随着时间的推移占用磁盘空间。在构建和安装端口之后，在端口骨架中运行 `make clean` 将清理临时的 `work` 目录。如果使用 Portmaster 安装端口，它会自动删除此目录，除非指定了 `-K` 选项。如果安装了 Portupgrade ，此命令将删除在本地 Ports Collection 副本中找到的所有 `work` 目录：
...
此外，随着时间的推移，过时的源代码分发文件会在 `/usr/ports/distfiles` 中累积。要使用 Portupgrade 删除所有不再被任何端口引用的分发文件：
...
Portupgrade 可以删除系统上当前未安装任何端口引用的所有 distfiles 文件。
...
poudriere 是使用 package:ports-mgmt/poudriere[] 包或端口进行安装的。安装过程中包含一个示例配置文件 `/usr/local/etc/poudriere.conf.sample` 。将此文件复制到 `/usr/local/etc/poudriere.conf` 。编辑复制的文件以适应本地配置。
...
在一台计算机上， poudriere 可以使用多个配置，在多个 jail 中，从不同的端口树构建端口。这些组合的自定义配置被称为“集合”。在安装了 package:ports-mgmt/poudriere[] 或 package:ports-mgmt/poudriere-devel[] 之后，可以查看 man:poudriere[8] 的 CUSTOMIZATION 部分获取详细信息。
...
. 搜索一下链接 https://www.FreeBSD.org/support/ 中的端口是否有待修复的问题报告。如果有的话，实施建议的修复可能会解决这个问题。
. 向端口的维护者寻求帮助。在端口骨架中键入 `make maintainer` ，或者阅读端口的 `Makefile` 以找到维护者的电子邮件地址。记得在发送给维护者的电子邮件中包含错误之前的输出。
+
[NOTE]
====
一些端口不是由个人维护，而是由一个由邮件列表表示的团队维护者维护。其中许多（但不是全部）地址看起来像 mailto:freebsd-listname @ FreeBSD.org[freebsd-listname @ FreeBSD.org] 。在发送电子邮件时，请考虑这一点。

特别是，由 mailto:ports @ FreeBSD.org[ports @ FreeBSD.org] 维护的端口不由特定个人维护。相反，任何修复和支持都来自订阅该邮件列表的广大社区。我们始终需要更多的志愿者！
====
+
如果没有收到邮件回复，请按照 extref:{problem-reports}[编写 FreeBSD 问题报告] 中的说明，使用 Bugzilla 提交错误报告。
