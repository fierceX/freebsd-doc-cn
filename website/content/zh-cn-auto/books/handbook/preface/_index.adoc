---
description: 'FreeBSD 新手会发现本书的第一部分指导用户完成 FreeBSD 安装过程，并温和地介绍了支撑 UNIX 的概念和约定。'
...
FreeBSD 新手会发现本书的第一部分指导用户完成 FreeBSD 安装过程，并温和地介绍了 UNIX(R) 的概念和约定。阅读这一部分只需要有探索的愿望，以及接受新概念的能力。
...
* 链接： https://docs.FreeBSD.org[FreeBSD 文档门户] 已创建。
* crossref:wayland[wayland , Wayland] 已添加，其中包含在 FreeBSD 下安装和配置 Wayland 的信息。
...
* crossref:wine[wine , WINE] 已添加了关于如何在 FreeBSD 上运行 Windows(R) 应用程序的信息。
* 已添加 crossref:dtrace[dtrace , DTrace] ，其中包含有关强大的 DTrace 性能分析工具的信息。
* 已添加 crossref:filesystems[文件系统，其他文件系统] ，其中包含有关 FreeBSD 中非本机文件系统（例如 Sun(TM) 的 ZFS ）的信息。
* 已添加 crossref:audit[audit ,安全事件审计] ，以覆盖 FreeBSD 中的新审计功能并解释其用途。
* 已添加 crossref:virtualization[virtualization ,虚拟化] ，其中包含有关在虚拟化软件上安装 FreeBSD 的信息。
* 已添加 crossref:bsdinstall[bsdinstall ，安装 FreeBSD] ，以覆盖使用新的安装工具 bsdinstall 安装 FreeBSD 的内容。
...
* crossref:config[config-tuning ,配置和调优] 已经增加了关于 ACPI 电源和资源管理、 `cron` 系统实用程序以及更多内核调优选项的新信息。
* crossref:security[security , Security] 已经增加了关于虚拟私有网络（ VPN ）、文件系统访问控制列表（ ACLs ）和安全公告的新信息。
* crossref:mac[mac ,强制访问控制] 是本版本的新章节。它解释了 MAC 是什么，以及如何使用这种机制来保护 FreeBSD 系统。
* crossref:disks[disks , Storage] 已经增加了关于 USB 存储设备、文件系统快照、文件系统配额、文件和网络支持的文件系统以及加密磁盘分区的新信息。
* 已向 crossref:ppp-and-slip[ppp-and-slip , PPP] 添加了故障排除部分。
* crossref:mail[mail ,电子邮件] 已经扩展了关于使用替代传输代理、 SMTP 身份验证、 UUCP 、 fetchmail 、 procmail 和其他高级主题的新信息。
...
* crossref:advanced-networking[高级网络] 已经增加了关于在 FreeBSD 上使用蓝牙设备、设置无线网络和异步传输模式（ ATM ）网络的新信息。
...
* crossref:basics[basics , FreeBSD 基础知识] 已经扩展，包含了关于进程、守护进程和信号的额外信息。
* crossref:ports[ports ,安装应用程序：软件包和端口] 已经扩展，包含了关于二进制包管理的额外信息。
* x11 （ X 窗口系统）已经完全重写，重点是使用现代桌面技术，如 KDE 和 GNOME 在 XFree86(TM) 4.X 上。
* crossref:boot[boot , FreeBSD 引导过程] 已经扩展。
* crossref:disks[disks ,存储] 一章是由原本分为"磁盘"和"备份"两个章节合并而成的。我们认为将这些主题作为一个单独的章节呈现更容易理解。同时，还添加了一个关于 RAID （硬件和软件）的部分。
* crossref:serialcomms[serialcomms , Serial Communications] 已经完全重新组织和更新，适用于 FreeBSD 4.X/5.X 。
...
* crossref:mail[mail ,电子邮件] 已经扩展，包括更多关于配置 sendmail 的信息。
* crossref:linuxemu[linuxemu , Linux ®二进制兼容性] 已经扩展，包括了安装 Oracle(R) 和 SAP(R) R/3(R) 的信息。
* 本第二版涵盖了以下新主题：
...
_crossref:basics[基础知识， FreeBSD 基础]_::
...
_crossref:kernelconfig[内核配置,配置 FreeBSD 内核]_::
...
描述了 FreeBSD 中的监狱框架以及监狱相对于传统的 chroot 支持的改进。
...
描述了如何在 FreeBSD 上配置和使用 Sun(TM) 的 DTrace 工具。动态跟踪可以通过进行实时系统分析来帮助定位性能问题。
