...
有关如何进行高级网络配置的更多信息，请参阅 crossref:advanced-networking[高级网络配置， Advanced Networking] 。
...
* `em0` ：以太网接口。
* `lo0` ：循环接口是一种软件环回机制，可用于性能分析、软件测试和 / 或本地通信。有关更多信息，请参阅 man:lo[4] 。
...
[[network-ipv6]]
=== IPv6 是 Internet Protocol version 6 的缩写，即互联网协议第六版。它是互联网上的一种网络协议，用于为设备在网络中进行唯一标识和定位。 IPv6 是 IPv4 的后继版本，旨在解决 IPv4 地址空间不足的问题。与 IPv4 相比， IPv6 采用 128 位地址，提供了更大的地址空间，可以支持更多的设备连接到互联网。 IPv6 还提供了更好的安全性、可靠性和性能。
...
Anycast 是一种网络通信技术，它允许多个服务器共享同一个 IP 地址。当用户发送请求时，网络会将请求路由到最近的可用服务器，以提供更快的响应时间和更好的负载均衡。 Anycast 广泛应用于互联网服务，如域名系统（ DNS ）和内容分发网络（ CDN ），以提高网络性能和可靠性。::
...
|`::` 是 asciidoc 中的一个特殊标记，用于表示一个块级元素的开始。
|128 位
|未指定的
|IPv4 中等同于 `0.0.0.0` 的地址。

|`::1` 是 IPv6 环回地址的表示形式。在 IPv6 中，环回地址用于将数据包发送到本地主机，类似于 IPv4 中的 `127.0.0.1` 。 `::1` 表示的是 IPv6 环回地址的最简形式。
|128 位
|环回地址
...
|`::ff:xx:xx:xx:xx` 是一个以双冒号开头的字符串，通常用于表示 MAC 地址。 MAC 地址是用于识别网络设备的唯一标识符。每个 MAC 地址由六个十六进制数字对（每个数字对由两个字符组成）组成，用冒号分隔。在这个例子中，"xx"代表未知的十六进制数字。
|96 位
|IPv4 映射 IPv6 地址
|低 32 位是不支持 IPv6 的主机的 IPv4 地址。

|`fe80::/10` 是一个 IPv6 地址的网络前缀。它表示一个具有 10 位网络前缀的地址范围，其中 `fe80` 是网络前缀的起始地址。 IPv6 地址的网络前缀用于标识一个特定的网络或子网。
|10 位
|链路本地
|在 IPv4 中，等同于 169.254.0.0/16 的地址。

|`fc00::/7` 是一个 IPv6 地址块，它是专门用于私有网络的地址范围。在 IPv6 中，私有网络地址用于在局域网或组织内部使用，不会被路由器转发到公共互联网。这个地址块的前 7 位是固定的，即以 `fc` 开头，后面的 9 位可以根据需要进行分配和使用。
|7 位
|唯一本地
|唯一本地地址用于本地通信，只能在一组合作站点内进行路由。

|`ff00::` 是 IPv6 地址中的一个保留地址，用于多播通信。
|8 位
|多播
| 

|``2000::-3fff::`` 是一个 IPv6 地址范围的表示方式。它表示从 2000 到 3fff 之间的所有 IPv6 地址。
|3 位
|全局单播
...
大多数无线网络都基于 IEEE(R) 802.11 标准。

FreeBSD 支持使用 802.11a 、 802.11b 、 802.11g 和 802.11n 的网络。

[NOTE]
====
在 FreeBSD 上，目前正在开发 [802.11ac] 支持。
====
...
在 FreeBSD 上，可以以“故障转移”的方式将两个甚至更多的网络接口组合在一起。这种配置类型使用一组网络接口中最优先和可用的连接，并且当链路状态发生变化时，操作系统会自动切换。
...
[[dns]]
== DNS （ Domain Name System ）是一种用于将域名转换为 IP 地址的互联网服务。它充当了互联网上的电话簿，将人类可读的域名（例如 www.example.com ）转换为计算机可理解的 IP 地址（例如 192.0.2.1 ）。 DNS 还负责将域名解析为其他类型的记录，如电子邮件服务器和其他网络服务的地址。通过使用 DNS ，用户可以轻松地访问互联网上的各种资源，而无需记住复杂的 IP 地址。

DNS 可以被理解为一个链接： [电话簿](https://en.wikipedia.org/wiki/Telephone_directory) ，其中将 IP 地址与主机名进行互相识别。
...
默认情况下， [/etc/nsswitch.conf](文件) 文件的 hosts 部分将如下所示：
...
|`nameserver` 是一个计算机网络术语，指的是域名系统（ DNS ）服务器的地址。 DNS 服务器用于将域名转换为相应的 IP 地址，以便在互联网上进行通信。
...
[TIP]
====
在发送错误报告之前，始终要检查链接中的硬件说明： https://www.freebsd.org/releases/[FreeBSD 发布页面] ，将 FreeBSD 版本更新到最新的 STABLE 版本，检查邮件列表存档，并在互联网上进行搜索。
====
...
`ping: sendto: Permission denied` 错误消息通常是由于配置错误的防火墙引起的。如果在 FreeBSD 上启用了防火墙但没有定义规则，那么默认策略是拒绝所有流量，甚至是 man:ping[8] 。有关更多信息，请参考 crossref:firewalls[firewalls , Firewalls] 。
