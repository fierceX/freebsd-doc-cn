...
* [.filename]#/etc/rc# 脚本。
* 网络术语。
* 安装额外的第三方软件（crossref:ports[ports,安装应用程序：软件包和 Ports]）。
...
配置 inetd 是通过编辑 [.filename]#/etc/inetd.conf# 文件来完成的。该配置文件的每一行表示一个可以由 inetd 启动的应用程序。默认情况下，每一行都以注释（`+#+`）开头，表示 inetd 不监听任何应用程序。要配置 inetd 监听应用程序的连接，请删除该应用程序行开头的 `+#+`。

在保存您的编辑后，通过编辑 [.filename]#/etc/rc.conf# 配置 inetd 在系统启动时启动：
...
一旦 inetd 启动，每当对 [.filename]#/etc/inetd.conf# 文件进行修改时，需要通知它。
...
{wait|nowait}[/max-child[/max-connections-per-ip-per-minute[/max-child-per-ip]]]:: 在这个字段中，必须指定 `wait` 或 `nowait`。`max-child`、`max-connections-per-ip-per-minute` 和 `max-child-per-ip` 是可选的。
+
...
-c 最大值::
指定每个服务的默认最大并发调用数，其中默认值为无限制。可以通过在 [.filename]#/etc/inetd.conf# 中使用 `max-child` 来对每个服务进行单独设置。

-C 速率::
指定单个 IP 地址每分钟可以调用服务的默认最大次数。可以通过在 [.filename]#/etc/inetd.conf# 中使用 `max-connections-per-ip-per-minute` 来覆盖每个服务的基础上。

-R 速率::
指定一个服务在一分钟内可以被调用的最大次数，其中默认值为 `256` 。设置为 `0` 表示允许无限次调用。

-s 最大值::
指定单个 IP 地址在任何一次中可以调用服务的最大次数，其中默认值为无限。可以通过在 [.filename]#/etc/inetd.conf# 中使用 `max-child-per-ip` 来覆盖每个服务的基础设置。
...
NFS 服务器将共享的文件系统在 [.filename]#/etc/exports# 文件中指定。该文件中的每一行都指定了要导出的文件系统，客户端对该文件系统的访问权限以及任何访问选项。在向该文件添加条目时，每个导出的文件系统、其属性和允许的主机必须出现在同一行上。如果条目中没有列出客户端，则任何网络上的客户端都可以挂载该文件系统。
...
`-ro` 标志使文件系统变为只读，防止客户端对导出的文件系统进行任何更改。此示例假设主机名要么在 DNS 中，要么在 [.filename]#/etc/hosts# 文件中。如果网络没有 DNS 服务器，请参考 `man:hosts[5]`。

下面的示例通过 IP 地址将 [.filename]#/home# 导出给三个客户端。这对于没有 DNS 或 [.filename]#/etc/hosts# 条目的网络非常有用。 `-alldirs` 标志允许子目录作为挂载点。换句话说，它不会自动挂载子目录，但会允许客户端根据需要挂载所需的目录。
...
要在启动时启用 NFS 服务器所需的进程，请将以下选项添加到 [.filename]#/etc/rc.conf# 文件中：
...
每当 NFS 服务器启动时，mountd 也会自动启动。然而，mountd 只在启动时读取 [.filename]#/etc/exports# 文件。为了使后续对 [.filename]#/etc/exports# 文件的编辑立即生效，强制 mountd 重新读取它：
...
要启用 NFS 客户端，请在每个客户端的 [.filename]#/etc/rc.conf# 文件中设置此选项：
...
现在，位于 [.filename]#/home# 目录中的文件和目录将在 `client` 上的 [.filename]#/mnt# 目录中可用。

要在每次客户端启动时挂载远程文件系统，请将其添加到 [.filename]#/etc/fstab# 文件中：
...
主要的 autofs 配置文件是 [.filename]#/etc/auto_master#。它将各个映射分配给顶级挂载点。有关 [.filename]#auto_master# 和映射语法的解释，请参考 `man:auto_master[5]` 。
...
`showmount` 命令的输出显示 [.filename]#/usr# 目录被导出。当切换到 [.filename]#/host/foobar/usr# 目录时， man:automountd[8] 会拦截请求并尝试解析主机名 `foobar`。如果成功，man:automountd[8] 会自动挂载源导出。
...
本节描述了一个示例的 NIS 环境，该环境由 15 台 FreeBSD 机器组成，没有集中的管理点。每台机器都有自己的 [.filename]#/etc/passwd# 和 [.filename]#/etc/master.passwd# 文件。这些文件只能通过手动干预来保持同步。目前，当在实验室中添加用户时，这个过程必须在所有 15 台机器上重复执行。
...
根据环境需求，设置主 NIS 服务器可以相对简单。由于 FreeBSD 提供了内置的 NIS 支持，只需要通过将以下行添加到 [.filename]#/etc/rc.conf# 来启用 NIS ：
...
一个既是服务器又是客户端的服务器可以通过在 [.filename]#/etc/rc.conf# 中添加以下额外的行来强制绑定到特定的服务器：
...
保存编辑后，输入 `/etc/netstart` 来重新启动网络并应用在 [.filename]#/etc/rc.conf# 中定义的值。在初始化 NIS 映射之前，启动 man:ypserv[8]：
...
NIS 映射是从 NIS 主服务器上的配置文件 [.filename]#/etc# 生成的，只有一个例外：[.filename]#/etc/master.passwd#。这是为了防止密码传播到 NIS 域中的所有服务器。因此，在初始化 NIS 映射之前，请配置主密码文件：
...
要设置一个 NIS 从服务器，登录到从服务器并像主服务器一样编辑 [.filename]#/etc/rc.conf#。不要生成任何 NIS 映射，因为这些映射已经存在于主服务器上。在从服务器上运行 `ypinit` 时，使用 `-s`（代表从服务器）而不是 `-m` （代表主服务器）。此选项除了域名外，还需要提供 NIS 主服务器的名称，如下例所示：
...
这将在从服务器上生成一个名为 [.filename]#/var/yp/test-domain# 的目录，其中包含 NIS 主服务器映射的副本。在每个从服务器上添加这些 [.filename]#/etc/crontab# 条目将强制从服务器将其映射与主服务器上的映射同步：
...
. 要从 NIS 服务器导入所有可能的组条目，请将以下行添加到 [.filename]#/etc/group# 文件中：
+
[.programlisting]
....
+:*::
...
crossref:security[tcpwrappers,"TCP Wrapper"] 是一种替代的访问控制机制，用于提供对 [.filename]#securenets# 的访问控制。虽然这两种访问控制机制都增加了一定的安全性，但它们都容易受到“IP 欺骗”攻击的影响。所有与 NIS 相关的流量都应该在防火墙上被阻止。
...
第一步是初始化 NIS 的 `netgroup` 映射。在 FreeBSD 中，默认情况下不会创建此映射。在 NIS 主服务器上，使用编辑器创建一个名为 [.filename]#/var/yp/netgroup# 的映射。
...
请确保在 `+@IT_EMP:::::::::` _之后_放置额外的空行。否则，从 NIS 导入的所有用户帐户将将 [.filename]#/usr/sbin/nologin# 作为其登录 shell，而没有人将能够登录到系统。
...
机器特定的网络组定义是处理策略变更的另一种可能性。在这种情况下，每个系统的 [.filename]#/etc/master.passwd# 文件包含两行以"+"开头的内容。第一行添加了一个允许登录到该机器的帐户的网络组，第二行添加了所有其他帐户，并将其 shell 设置为 [.filename]#/usr/sbin/nologin#。建议使用主机名的大写版本作为网络组的名称：
...
一旦在所有机器上完成了这个任务，就不再需要再次修改本地版本的 [.filename]#/etc/master.passwd# 文件了。所有后续的更改都可以通过修改 NIS 映射来处理。以下是一个可能的 `netgroup` 映射的示例：
...
在 extref:{linux-users}[package, software] 中启用了一组大量的默认选项。通过运行 `pkg info openldap-server` 来查看它们。如果它们不足够（例如需要 SQL 支持），请考虑使用适当的 crossref:ports[ports-using, framework] 重新编译该 port。
...
http://www.openldap.org/doc/admin24/slapdconf2.html[配置示例] 可以在 [.filename]#slapd.ldif# 中找到，并且还可以在 [.filename]#/usr/local/etc/openldap/slapd.ldif.sample# 中找到。选项在 slapd-config(5) 中有文档记录。[.filename]#slapd.ldif# 的每个部分，就像所有其他 LDAP 属性集一样，都通过 DN 唯一标识。请确保在 `dn:` 语句和所需部分的末尾之间没有空行。在下面的示例中，将使用 TLS 来实现安全通道。第一部分表示全局配置：
...
另一个部分专门介绍了 _配置后端_，这是以全局超级用户的身份访问 OpenLDAP 服务器配置的唯一方式。
...
这个 http://www.openldap.org/devel/gitweb.cgi?p=openldap.git;a=tree;f=tests/data/regressions/its8444;h=8a5e808e63b0de3d2bdaf2cf34fecca8577ca7fd;hb=HEAD[仓库] 包含了四个 [.filename]#slapd.ldif# 的示例。要将现有的 [.filename]#slapd.conf# 转换为 [.filename]#slapd.ldif# ，请参考 http://www.openldap.org/doc/admin24/slapdconf2.html[此页面] （请注意，这可能会引入一些无用的选项）。
...
只有全局超级用户才能对 _global configuration_ 部分进行修改。例如，假设最初指定了选项 `olcTLSCipherSuite: HIGH:MEDIUM:SSLv3`，现在必须删除该选项。首先，创建一个包含以下内容的文件：
...
当被询问时，请提供在 _configuration backend_ 部分选择的密码。用户名不是必需的：在这里，`cn=config` 表示要修改的数据库部分的 DN。或者，使用 `ldapmodify` 删除数据库的单行，`ldapdelete` 删除整个条目。
...
如果系统在安装过程中配置为使用 DHCP，则此行可能已经存在。在这些示例中，将 _fxp0_ 替换为要动态配置的接口的名称，如 crossref:config[config-network-setup,“设置网络接口卡”] 中所述。
...
本节演示了如何使用 Internet Systems Consortium (ISC) 实现的 DHCP 服务器将 FreeBSD 系统配置为 DHCP 服务器。可以使用 package:net/isc-dhcp44-server[] 软件包或 port 安装此实现及其文档。

package:net/isc-dhcp44-server[] 将安装一个示例配置文件。将 [.filename]#/usr/local/etc/dhcpd.conf.example# 复制到 [.filename]#/usr/local/etc/dhcpd.conf#，并对这个新文件进行任何编辑。
...
+
这个守护进程用于高级环境中，其中一个 DHCP 服务器将客户端的请求转发到另一个位于不同网络上的 DHCP 服务器。如果需要这个功能，请安装 package:net/isc-dhcp44-relay[] 包或 port。安装包括了提供更多详细信息的 dhcrelay(8)。
...
要启用 Unbound ，请将以下内容添加到 [.filename]#/etc/rc.conf# 文件中：
...
在新的 Unbound 配置中，任何已存在的名字服务器在 [.filename]#/etc/resolv.conf# 文件中将被配置为转发器。
...
这将负责更新 [.filename]#/etc/resolv.conf# 以便查询 DNSSEC 安全域名能够正常工作。例如，运行以下命令来验证 FreeBSD.org 的 DNSSEC 信任树：
...
=== 权威名称服务器配置
FreeBSD 在基本系统中不提供权威名称服务器软件。鼓励用户安装第三方应用程序，如 package:dns/nsd[] 或 package:dns/bind918[] 包或 port。
...
在 FreeBSD 中，主要的 Apache HTTP 服务器配置文件被安装在 [.filename]#/usr/local/etc/apache2x/httpd.conf#，其中 _x_ 代表版本号。这个 ASCII 文本文件以 `+#+` 开头的行作为注释。最经常修改的指令有：
...
要在系统启动时启动 Apache，请将以下行添加到 [.filename]#/etc/rc.conf# 文件中：
...
如果要使用非默认选项启动 Apache，则可以将以下行添加到 [.filename]#/etc/rc.conf# 以指定所需的标志：
...
Apache 使用模块来增强基本服务器提供的功能。请参考 http://httpd.apache.org/docs/current/mod/[http://httpd.apache.org/docs/current/mod/]，了解可用模块的完整列表和配置细节。

在 FreeBSD 中，一些模块可以使用 package:www/apache24[] port 进行编译。在 [.filename]#/usr/ports/www/apache24# 目录下输入 `make config` 命令可以查看可用的模块以及默认启用的模块。如果模块没有与端口一起编译，FreeBSD Ports Collection 提供了一种简单的方法来安装许多模块。本节介绍了三个最常用的模块。
...
可以使用 package:www/mod_perl2[] 包或 port 安装 [.filename]#mod_perl#。有关使用此模块的文档可以在 http://perl.apache.org/docs/2.0/index.html[http://perl.apache.org/docs/2.0/index.html] 找到。
...
默认情况下，PHP 将不会被启用。要使其生效，需要将以下行添加到位于 [.filename]#/usr/local/etc/apache24# 目录中的 Apache 配置文件中：
...
最后，要对 PHP 的全局配置进行任何更改，需要使用一个文档完善的文件，安装在 [.filename]#/usr/local/etc/php.ini#。在安装时，该文件不存在，因为有两个版本可供选择，一个是 [.filename]#php.ini-development#，另一个是 [.filename]#php.ini-production#。这些是管理员在部署过程中的起点。
...
[NOTE]
====
有一个单独的 [.filename]#mod_http2# port 可用。它的存在是为了更快地提供安全性和错误修复，而不是使用捆绑的 [.filename]#apache24# port 安装的模块。它不是必需的 HTTP2 支持，但可供选择。安装后，应在 Apache 配置中使用 [.filename]#mod_h2.so# 替代 [.filename]#mod_http2.so#。
====
...
Ruby on Rails 是另一个开源的 Web 框架，提供了完整的开发堆栈。它经过优化，可以使 Web 开发人员更加高效和能够快速编写强大的应用程序。在 FreeBSD 上，可以使用 package:www/rubygem-rails[] 包或 port 进行安装。
...
最重要的配置步骤是决定哪些账户将被允许访问 FTP 服务器。FreeBSD 系统有许多系统账户，不应该允许 FTP 访问。不允许任何 FTP 访问的用户列表可以在 [.filename]#/etc/ftpusers# 中找到。默认情况下，它包括系统账户。可以添加其他不允许访问 FTP 的用户。
...
ftpd 守护进程使用 man:syslog[3] 来记录日志消息。默认情况下，系统日志守护进程将会将与 FTP 相关的消息写入到 [.filename]#/var/log/xferlog# 文件中。FTP 日志的位置可以通过修改 [.filename]#/etc/syslog.conf# 文件中的以下行来进行更改：
...
在 FreeBSD 上，可以使用 package:net/samba413[] 端口或 port 来安装 Samba 客户端库。该客户端提供了 FreeBSD 系统访问 Microsoft(R) Windows(R) 网络中的 SMB/CIFS 共享的能力。

通过安装相同的 package:net/samba413[] 包或 port， FreeBSD 系统也可以配置为 Samba 服务器。这使得管理员可以在 FreeBSD 系统上创建 SMB/CIFS 共享，这些共享可以被运行 Microsoft(R) Windows(R) 或 Samba 客户端库的客户端访问。

=== 服务器配置

Samba 的配置文件位于 [.filename]#/usr/local/etc/smb4.conf#。在使用 Samba 之前，必须先创建该文件。
...
在 [.filename]#/usr/local/etc/smb4.conf# 中添加描述网络的设置：
...
要在启动时启用 Samba ，请将以下行添加到 [.filename]#/etc/rc.conf# 文件中：
...
ntpd 读取 [.filename]#/etc/ntp.conf# 文件来确定要查询的 NTP 服务器。建议选择多个 NTP 服务器，以防其中一个服务器无法访问或其时钟不可靠。当 ntpd 接收到响应时，它会优先选择可靠的服务器而不是不可靠的服务器。被查询的服务器可以是本地网络的服务器、由 ISP 提供的服务器，或者从一个 http://support.ntp.org/bin/view/Servers/WebHome[在线的公共可访问的 NTP 服务器列表] 中选择。选择公共 NTP 服务器时，选择地理位置接近的服务器并查看其使用政策。`pool` 配置关键字从服务器池中选择一个或多个服务器。有一个 http://support.ntp.org/bin/view/Servers/NTPPoolServers[在线的公共可访问的 NTP 池列表]，按地理区域组织。此外，FreeBSD 提供了一个由项目赞助的池，即 `0.freebsd.pool.ntp.org`。
...
默认情况下，NTP 服务器对任何网络主机都是可访问的。`restrict` 关键字用于控制哪些系统可以访问该服务器。支持多个 `restrict` 条目，每个条目都可以细化之前语句中给出的限制。示例中显示的值允许本地系统完全查询和控制访问，而只允许远程系统查询时间。有关更多详细信息，请参阅 man:ntp.conf[5] 中的 `访问控制支持（Access Control Support）` 子节。
...
==== 在 [.filename]#/etc/rc.conf# 文件中的 NTP 条目

将 `ntpd_enable=YES` 设置为在启动时启动 ntpd。一旦将 `ntpd_enable=YES` 添加到 [.filename]#/etc/rc.conf# 文件中，可以通过输入以下命令立即启动 ntpd 而无需重新启动系统：
...
将 `ntpd_flags=` 设置为包含所需的任何其他 ntpd 标志，但避免使用这些由 [.filename]#/etc/rc.d/ntpd# 内部管理的标志。
...
ntpd 在正常运行时不需要与互联网保持永久连接。然而，如果配置了 PPP 连接以按需拨号，应该阻止 NTP 流量触发拨号或保持连接活动。这可以通过在 [.filename]#/etc/ppp/ppp.conf# 文件中使用 `filter` 指令进行配置。例如：
...
有关更多详细信息，请参阅 man:ppp[8] 中的 `PACKET FILTERING` 部分以及 [.filename]#/usr/share/examples/ppp/# 中的示例。
...
在 iSCSI 术语中，共享存储的系统被称为 _目标（target）_。存储可以是物理磁盘，也可以是表示多个磁盘或物理磁盘部分的区域。例如，如果磁盘使用 ZFS 格式化，可以创建一个 zvol 作为 iSCSI 存储使用。

访问 iSCSI 存储的客户端被称为 _发起者（initiators）_。对于发起者来说，通过 iSCSI 可用的存储看起来像一个未格式化的原始磁盘，称为逻辑单元 (LUN) 。磁盘的设备节点出现在 [.filename]#/dev/# 中，设备必须单独进行格式化和挂载。
...
要配置一个 iSCSI 目标，需要创建一个名为 [.filename]#/etc/ctl.conf# 的配置文件，然后在 [.filename]#/etc/rc.conf# 中添加一行，以确保 man:ctld[8] 守护进程在启动时自动启动，最后启动守护进程。
...
下一节定义了 LUN。对于发起者来说，每个 LUN 将被视为一个独立的磁盘设备。每个目标可以定义多个 LUN。每个 LUN 由一个数字标识，其中 LUN 0 是必需的。`path /data/target0-0` 行定义了支持 LUN 的文件或 zvol 的完整路径。在启动 man:ctld[8] 之前，该路径必须存在。第二行是可选的，用于指定 LUN 的大小。
...
[NOTE]
====
本节中描述的 iSCSI 发起者从 FreeBSD 10.0-RELEASE 开始得到支持。要使用旧版本中提供的 iSCSI initiator，请参考 man:iscontrol[8] 。
====

iSCSI 发起者要求 man:iscsid[8] 守护进程正在运行。该守护进程不使用配置文件。要在启动时自动启动它，请将以下行添加到 [.filename]#/etc/rc.conf# 文件中：
...
要使用配置文件进行连接，请创建一个名为 [.filename]#/etc/iscsi.conf# 的文件，并将其内容设置为以下内容：
...
要使发起者自动连接到 [.filename]#/etc/iscsi.conf# 中的所有目标，请将以下内容添加到 [.filename]#/etc/rc.conf# 中：
