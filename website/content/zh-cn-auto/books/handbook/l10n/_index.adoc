...
|asciidoc 是一种轻量级的标记语言，用于编写技术文档和文档生成工具。它使用简单的文本格式来描述文档的结构和样式，并支持各种功能，如标题、列表、链接、代码块和表格。 asciidoc 还可以与其他工具集成，如 Git 和 Jenkins ，以便更好地管理和自动化文档的生成和发布过程。无论是初学者还是专业人士， asciidoc 都是一个强大而灵活的工具，可以帮助您创建高质量的技术文档。
|英语，美国

|ru_RU
|俄罗斯，俄罗斯

|你是一個翻譯引擎，精通從英文到中文的翻譯，並且擁有計算機相關領域知識，知道 asciidoc 格式的規範。請將下面的文本翻譯為中文，不需要解釋，請直接返回譯文。
|繁體中文，台灣
|===
...
|`cons25l1` 是一个计算机术语，指的是 Linux 内核中的一个选项。它是 Linux 控制台的一种显示模式，用于支持 25 行的文本显示，并且只能显示一种颜色。

|ISO8859-2 是一种字符编码标准，用于表示中欧和东欧语言中的字符。它是 ISO/IEC 8859 系列的一部分，也被称为 Latin-2 编码。 ISO8859-2 支持包括捷克语、斯洛伐克语、波兰语、匈牙利语和罗马尼亚语在内的多种语言。
|`cons25l2` 是一个计算机术语，指的是一种基于 25 行的低分辨率控制台显示器。

|ISO8859-7 是一种字符编码标准，用于表示希腊语字符集。
|`cons25l7` 是一个计算机术语，它可能指的是一个特定的终端类型或终端模拟器。具体而言， `cons25l7` 可能代表一个具有 25 行和 7 列的字符终端。这种终端通常用于文本界面的显示和输入，例如在命令行界面中使用。

|KOI8-R 是一种字符编码，用于将俄语文本转换为计算机可读的格式。它是基于 KOI8 编码的一种变体，主要用于俄罗斯和其他一些国家的计算机系统中。 KOI8-R 编码包含了俄语字母、标点符号和其他特殊字符，可以在计算机系统中正确显示和处理俄语文本。
|`cons25r` 是一个计算机术语，指的是一个用于控制终端显示的设备驱动程序。它通常用于支持终端的 25 行显示模式。

|KOI8-U 是一种字符编码，用于表示乌克兰语和白俄罗斯语等斯拉夫语言。它是 KOI8 编码的一个变种，支持乌克兰语和白俄罗斯语中的特殊字符。 KOI8-U 编码在计算机领域中广泛使用，特别是在早期的 Unix 系统和电子邮件中。
|`cons25u` 是一个计算机术语，指的是一种特定的终端类型。

|CP437 （ VGA 默认）
|`cons25` 是一个计算机术语，指的是一种终端类型。它是一种基于文本的终端，具有 25 行和 80 列的显示屏。这种终端通常用于命令行界面和文本编辑器等应用程序。

|美国标准信息交换码（ US-ASCII ）
|`cons25w` 是一个计算机术语，指的是一个具有 25 列字符宽度的控制台或终端。
|===
