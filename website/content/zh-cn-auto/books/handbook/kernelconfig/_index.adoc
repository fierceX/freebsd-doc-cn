---
description: '本章介绍了如何配置 FreeBSD 内核。包括何时构建自定义内核，如何进行硬件清单，如何自定义内核配置文件等等。'
...
内核模块存在于 [/boot/kernel] 目录中，并且可以使用 man:kldload[8] 命令动态加载到运行中的内核中。大多数内核驱动程序都有可加载的模块和手册页。例如， man:ath[4] 无线网络驱动程序在其手册页中包含以下信息：
...
[[kernelconfig-noboot]]
内核无法启动::
如果新内核无法启动或无法识别设备，请不要惊慌！幸运的是， FreeBSD 有一个出色的机制可以从不兼容的内核中恢复。只需在 FreeBSD 引导加载程序中选择要引导的内核即可。当系统启动菜单出现时，可以通过选择“进入加载程序提示符”选项来访问它。在提示符下，输入 `boot _kernel.old_` ，或者任何其他已知可以正确引导的内核的名称。
+
...
如果内核版本与系统实用程序构建的版本不同，例如，在 -RELEASE 系统上安装了从 -CURRENT 源构建的内核，那么许多系统状态命令（如 man:ps[1] 和 man:vmstat[8] ）将无法工作。为了解决这个问题，应该使用与内核相同版本的源代码树交叉引用 :cutting-edge[makeworld ，重新编译和安装整个系统] 。在操作系统中使用与其余部分不同版本的内核从来都不是一个好主意。
