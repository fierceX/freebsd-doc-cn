...
FreeBSD 支持多种架构，包括 amd64 、 ARM(R) 、 RISC-V(R) 和 PowerPC(R) 。根据不同的架构和平台，可以下载不同的镜像来安装或直接运行 FreeBSD 。

图像类型有：

* 虚拟机磁盘镜像，例如 `qcow2` 、 `vmdk` 、 `vhd` 和原始设备镜像。这些不是安装镜像，而是已经预装了 FreeBSD 并准备好进行后安装任务的镜像。虚拟机镜像在云环境中也被广泛使用。
* SD 卡镜像，用于树莓派等嵌入式系统。这些文件必须解压缩并以原始镜像的形式写入 SD 卡，从中启动板子。
...
* 如何排除安装失败的问题。
* 在进行安装之前，如何访问 FreeBSD 的实时版本。

[[bsdinstall-hardware]]
== 最低硬件要求

安装 FreeBSD 的硬件要求因架构和版本而异。支持 FreeBSD 发布的硬件架构和设备可以在链接： https://www.FreeBSD.org/releases/[FreeBSD 发布信息] 页面上找到。链接： https://www.FreeBSD.org/where/[FreeBSD 下载页面] 还提供了选择不同架构的正确镜像的建议。
...
+
在 i386 和 amd64 架构中，可以使用两种分区方案将磁盘分成多个分区。传统的主引导记录（ MBR ）保存了一个包含最多四个主分区的分区表。出于历史原因， FreeBSD 将这些主分区称为切片。其中一个主分区可以成为包含多个逻辑分区的扩展分区。 GUID 分区表（ GPT ）是一种较新且更简单的磁盘分区方法。常见的 GPT 实现允许每个磁盘最多有 128 个分区，消除了逻辑分区的需要。
+
...
FreeBSD 安装文件可以在链接 https://www.FreeBSD.org/where/[FreeBSD 下载页面] 上找到。每个安装文件的名称包括 FreeBSD 的发布版本、架构和文件类型。
...
* `*-dvd1.iso*` ：该文件包含安装 FreeBSD 所需的所有文件，包括其源代码和 Ports Collection 。它还包含一组流行的二进制软件包，用于安装窗口管理器和一些应用程序，以便可以在没有连接到互联网的情况下从介质上安装完整的系统。该文件应该被刻录到光盘介质上。
...
在下载其中一个文件（或两个文件）之后，计算图像文件的_checksum_并将其与_checksum_文件中显示的_checksum_进行比较。请注意，您需要将计算出的_checksum_与正确的文件进行比较，因为它们对应于两种不同的算法： SHA256 和 SHA512 。 FreeBSD 提供了可以用于计算_checksum_的 man:sha256[1] 和 man:sha512[1] 。其他操作系统也有类似的程序。

在 FreeBSD 中，可以通过执行 man:sha256sum[1] （和 man:sha512sum[1] ）来自动验证_checksum_。
...
` \ *memstick.img` 文件是一个内存棒的完整内容的镜像。它不能作为文件复制到目标设备上。有几个应用程序可用于将 `*.img` 写入 USB 内存棒。本节介绍其中两个实用程序。
...
....
+
如果此命令失败，请验证 USB 存储设备未挂载，并且设备名称是磁盘而不是分区。
+
...
. * 使用图像写入器写入图像 * 
+
双击 Win32DiskImager 图标启动程序。确保在“设备”下显示的驱动器字母是存储器的驱动器。点击文件夹图标并选择要写入存储器的镜像。点击 btn:[保存] 接受镜像文件名。确认一切正确，并且存储器上没有其他窗口中打开的文件夹。当一切准备就绪时，点击 btn:[写入] 将镜像文件写入存储器。
====
...
本节描述了如何从使用 <<bsdinstall-installation-media>> 中的说明准备的安装介质引导系统。使用可启动的 USB 闪存驱动器时，在打开计算机之前插入 USB 闪存驱动器。从 CD 或 DVD 引导时，打开计算机并在第一次机会时插入介质。如何配置系统以从插入的介质引导取决于体系结构。
...
.FreeBSD 引导加载程序菜单
image::bsdinstall-newboot-loader-menu.png%20是一个文件名，无法直接翻译。["FreeBSD 引导加载程序菜单"]
...
* ` 启动多用户模式 ` ：这将继续 FreeBSD 的启动过程。如果启动计时器已经暂停，请按下 kbd:[1] 、大写或小写的 kbd:[B] 或 kbd:[Enter] 键。
* ` 单用户启动 `: 这种模式可以用于修复已有的 FreeBSD 安装，如 crossref:boot[boot-singleuser ,"单用户模式"] 中所述。按下键盘上的 kbd:[2] 或大写或小写的 kbd:[S] 进入此模式。
* ` 进入加载器提示符 `: 这将使系统启动到一个包含有限数量低级命令的修复提示符。该提示符在 crossref:boot[boot-loader ,“ Stage Three ”] 中有描述。按下 kbd:[3] 或 kbd:[Esc] 键进入该提示符。
* `Reboot` ：重新启动系统。
...
* `ACPI 支持 `: 如果系统在启动过程中出现卡死的情况，请尝试将此选项切换为 ` 关闭 ` 。
* ` 安全模式 ` ：如果系统在启动时即使将 `ACPI 支持 ` 设置为 ` 关闭 ` 仍然出现卡顿的情况，请尝试将此选项设置为 ` 开启 ` 。
* `Single User` ：将此选项切换为 `On` ，以修复现有的 FreeBSD 安装，如 crossref:boot[boot-singleuser ,“单用户模式”] 中所述。一旦问题解决，将其设置为 `Off` 。
...
.欢迎菜单
image::bsdinstall-choose-mode.png%20是一个图片文件的名称，无法直接翻译。["FreeBSD 安装欢迎菜单"]
...
[TIP]
====
要查看启动消息，包括硬件设备探测，请按下大写或小写的键盘： [S] ，然后按下键盘： [Enter] 以访问 shell 。在 shell 提示符下，键入 `more /var/run/dmesg.boot` ，使用空格键滚动浏览消息。完成后，键入 `exit` 返回欢迎菜单。
====
...
.按键映射选择菜单
image::bsdinstall-keymap-10.png%20是一个图片文件的名称。["显示所有支持的键盘的按键映射选择菜单"]
...
.按键映射测试菜单
image::bsdinstall-keymap-testing.png%20是一个图片文件的名称。["按键映射测试菜单"]
...
.设置主机名
image::bsdinstall-config-hostname.png%20是一个文件名，无法直接翻译。["设置主机名"]
...
.选择要安装的组件
image::bsdinstall-config-components.png%20是一个图片文件的名称。["可以安装的不同组件。示例： base-dbg", lib32, ports, etc.]
...
* `ports` - FreeBSD Ports Collection 是一组文件，用于自动下载、编译和安装第三方软件包。 crossref:ports[ports ,安装应用程序：软件包和端口] 讨论了如何使用 Ports Collection 。
+
[WARNING]
====
...
请注意，从网络安装软件时，确保下载和安装的软件来自可信赖的来源，以避免安全风险。
image::bsdinstall-netinstall-files.png%20是一个文件名，无法直接翻译。["表示某些组件未找到，并将使用网络进行下载。"]
...
.分区选择
image::bsdinstall-zfs-partmenu.png%20是一个文件名，无法直接翻译。["显示不同的分区选项。示例：手动", Shell, etc.]

bsdinstall 为用户提供了四种分配磁盘空间的方法：

* `Auto (ZFS)` 分区创建一个基于 ZFS 的系统，可选择支持 GELI 加密用于_引导环境_。
...
在具有多个 SCSI 磁盘或多个在不同控制器上运行的 IDE 磁盘的较大系统上，建议在每个驱动器上配置交换空间，最多四个驱动器。交换分区的大小应该大致相同。内核可以处理任意大小，但内部数据结构会按照最大交换分区的 4 倍进行扩展。保持交换分区大小接近将允许内核在磁盘上最佳地条带化交换空间。较大的交换大小可能会引发有关总配置交换的内核警告消息。通过增加用于跟踪交换分配的内存量来提高限制，如警告消息所指示的那样。在被迫重新启动之前，从失控的程序中恢复可能更容易。
...
.从多个磁盘中进行选择
image::bsdinstall-part-guided-disk.png%20是一个文件名，无法直接翻译。["显示可以安装 FreeBSD 的磁盘列表"]

选择磁盘后，下一个菜单会提示选择是将整个磁盘安装，还是使用可用空间创建分区。如果选择 btn:[整个磁盘] ，将自动创建一个填满整个磁盘的通用分区布局。选择 btn:[分区] 将从磁盘上未使用的空间创建一个分区布局。

[[bsdinstall-part-entire-part]]
.选择整个磁盘或分区
image::bsdinstall-part-entire-part.png%20是一个图片文件的名称。["菜单询问用户是否想要使用磁盘上的所有可用空间，还是想要进行分区。"]
...
.选择分区方案
image::bsdinstall-part-manual-partscheme.png%20是一个图片文件的名称。["菜单显示用户存在的不同类型的分区，并要求选择其中之一。"]
...
.审查已创建的分区
image::bsdinstall-part-review.png%20是一个文件名，无法直接翻译。["显示已创建分区的菜单"]
...
.最终确认
image::bsdinstall-final-confirmation.png%20是一个文件名，无法直接翻译。["菜单向用户指示所有更改将被写入磁盘，并告知如果用户决定继续，现有数据将被永久删除。"]
...
.手动创建分区
image::bsdinstall-part-manual-create.png%20是一个图片文件的名称。["显示分区编辑器的菜单。"]

高亮显示安装驱动器（在此示例中为 `ada0` ），然后选择 btn:[Create] 以显示可用分区方案的菜单：

[[bsdinstall-part-manual-partscheme]]
.手动创建分区
image::bsdinstall-part-manual-partscheme.png%20是一个图片文件的名称。["菜单显示了不同类型的分区方案"]
...
|APM 是应用性能管理（ Application Performance Management ）的缩写，它是一种通过监控和分析应用程序的性能来优化和提升应用程序的方法。 APM 可以帮助开发人员和运维团队实时监控应用程序的性能指标，如响应时间、吞吐量和错误率，并提供详细的性能分析和故障排查工具。通过使用 APM ，可以及时发现和解决应用程序的性能问题，提高用户体验和满意度。
|Apple Partition Map ，由 PowerPC(R) 使用。

|BSD 是一种开放源代码的操作系统，它基于 UNIX 。 BSD 的名称来自于加州大学伯克利分校（ University of California , Berkeley ）的计算机科学系。 BSD 操作系统具有稳定性、可靠性和安全性的特点，并且被广泛用于服务器和嵌入式系统等领域。 BSD 操作系统有多个不同的分支，包括 FreeBSD 、 OpenBSD 和 NetBSD 等。
|没有 MBR 的 BSD 标签，有时被称为“危险专用模式”，因为非 BSD 磁盘工具可能无法识别它。

|GPT （ Generative Pre-trained Transformer ）是一种基于 Transformer 架构的预训练模型。它通过大规模的无监督学习来学习语言模型，然后可以用于各种自然语言处理任务，如文本生成、机器翻译和问答系统等。 GPT 模型在自然语言处理领域取得了很大的成功，并且被广泛应用于各种实际应用中。
|链接： https://en.wikipedia.org/wiki/GUID_Partition_Table[GUID 分区表] 。

|MBR （ Master Boot Record ）是计算机硬盘上的一个特殊区域，用于存储引导程序和分区表。它位于硬盘的第一个扇区，大小为 512 字节。 MBR 包含了引导程序的代码和分区表的信息，用于引导操作系统的加载和启动。在计算机启动时， BIOS 会读取 MBR ，并将控制权交给引导程序，从而启动操作系统。
...
.手动创建分区
image::bsdinstall-part-manual-addpart.png%20是一个图片文件的名称。["请求类型菜单", size, mountpoint and label for the new partition.]
...
[TIP]
====
正确的扇区对齐可以提供最佳性能，将分区大小设置为 4K 字节的倍数有助于确保在具有 512 字节或 4K 字节扇区的驱动器上进行对齐。通常，使用 1M 或 1G 的倍数作为分区大小是确保每个分区从 4K 的倍数开始的最简单方法。有一个例外：由于当前引导代码的限制，_freebsd-boot_分区的大小不应超过 512K 。
====
...
|`freebsd-boot` 是一个用于 FreeBSD 操作系统的引导加载程序。它负责在系统启动时加载操作系统内核，并提供启动选项和配置信息。 `freebsd-boot` 通常安装在系统的引导分区上，并与其他引导加载程序（如 GRUB 或 UEFI ）配合使用，以实现系统的引导和启动。
|`512K` 可以翻译为 `512 千字节 ` 。
| 
| 

|`freebsd-ufs` 是一种用于 FreeBSD 操作系统的文件系统类型。它是一种高性能的 Unix 文件系统，具有可靠性和稳定性。 FreeBSD 使用 `freebsd-ufs` 作为默认的文件系统类型，它支持许多先进的功能，如快照、日志记录和数据完整性保护。
|`2G` 是指第二代移动通信技术（ 2nd Generation ），也称为 2G 网络。它是在 1G （第一代）基础上发展而来的，具有更高的数据传输速度和更多的功能。 2G 网络使用数字信号进行通信，可以实现语音通话、短信传输和基本的数据传输。它为移动通信行业的发展奠定了基础，并为后续的 3G 和 4G 网络的出现铺平了道路。
|`/`
|`exrootfs` 是一个计算机术语，指的是扩展根文件系统（ Extended Root File System ）。

|`freebsd-swap` 是 FreeBSD 操作系统中用于交换空间的特殊文件系统类型。交换空间是计算机内存的扩展，用于临时存储不活跃的进程或内存中无法容纳的数据。在 FreeBSD 中， `freebsd-swap` 文件系统用于创建和管理交换空间。它可以通过设置交换分区或使用特殊的交换文件来实现。交换空间的大小和位置可以根据系统需求进行配置。
|`4G` 是第四代移动通信技术的简称，也被称为第四代无线宽带技术。它是对 3G 技术的升级和改进，提供更高的数据传输速度和更稳定的网络连接。 4G 技术支持更快的下载和上传速度，更低的延迟，以及更好的网络覆盖范围。它为移动设备用户提供了更好的互联网体验，支持高清视频流媒体、在线游戏和其他高带宽应用。
| 
|`exswap` 是一个计算机术语，指的是交换两个变量的值。在编程中，使用 `exswap` 可以通过交换两个变量的值来实现变量值的互换。

|`freebsd-ufs` 是一种用于 FreeBSD 操作系统的文件系统类型。它是一种高性能的 Unix 文件系统，具有可靠性和稳定性。 FreeBSD 使用 `freebsd-ufs` 作为默认的文件系统类型，它支持许多先进的功能，如快照、日志记录和数据完整性保护。
|`2G` 是指第二代移动通信技术（ 2nd Generation ），也称为 2G 网络。它是在 1G （第一代）基础上发展而来的，具有更高的数据传输速度和更多的功能。 2G 网络使用数字信号进行通信，可以实现语音通话、短信传输和基本的数据传输。它为移动通信行业的发展奠定了基础，并为后续的 3G 和 4G 网络的出现铺平了道路。
|`/var` 是一个在 Unix-like 操作系统中常见的目录。它是"variable"的缩写，用于存储在系统运行过程中会变化的文件，如日志文件、缓存文件和临时文件等。在这个目录下，可以找到许多子目录，每个子目录都有不同的用途。
...
|`freebsd-ufs` 是一种用于 FreeBSD 操作系统的文件系统类型。它是一种高性能的 Unix 文件系统，具有可靠性和稳定性。 FreeBSD 使用 `freebsd-ufs` 作为默认的文件系统类型，它支持许多先进的功能，如快照、日志记录和数据完整性保护。
|接受默认选项（使用磁盘的剩余空间）。
|`/usr` 是 Unix 操作系统中的一个目录，它是"User"的缩写，用于存放用户相关的文件和程序。在该目录下，通常包含了用户的个人文件、应用程序、库文件以及其他与用户相关的数据。
|`exusrfs` 是一个未知的术语或缩写，无法提供准确的翻译。
|===
====

在创建自定义分区后，选择 btn:[完成] 继续安装并转到 <<bsdinstall-fetching-distribution>> 。
...
* ` 安装 ` - 使用所选选项继续安装。
* ` 池类型 / 磁盘 ` - 配置 ` 池类型 ` 和组成池的磁盘。自动 ZFS 安装程序目前只支持创建单个顶级 vdev ，除非是条带模式。要创建更复杂的池，请使用 <<bsdinstall-part-shell>> 中的说明来创建池。
* ` 重新扫描设备 ` - 重新填充可用磁盘列表。
* ` 磁盘信息 ` - 此菜单可用于检查每个磁盘，包括其分区表和其他各种信息，如设备型号和序列号（如果可用）。
* ` 池名称 ` - 设置池的名称。默认名称为_zroot_。
* ` 强制使用 4K 扇区？ ` - 强制使用 4K 扇区。默认情况下，安装程序将自动创建与 4K 边界对齐的分区，并强制 ZFS 使用 4K 扇区。即使是 512 字节扇区的磁盘，这也是安全的，并且还有一个额外的好处，即确保在将来可以将 4K 扇区磁盘添加到在 512 字节磁盘上创建的池中，无论是作为额外的存储空间还是作为替换失败的磁盘。按下 kbd:[Enter] 键选择是否激活它。
* ` 加密磁盘？ ` - 加密磁盘允许用户使用 GELI 对磁盘进行加密。有关磁盘加密的更多信息，请参阅 crossref:disks[disks-encrypting-geli ，“使用 geli 进行磁盘加密”] 。按下 kbd:[Enter] 键选择是否激活它。
* ` 分区方案 ` - 选择分区方案。在大多数情况下， GPT 是推荐的选项。按下 kbd:[Enter] 键来在不同选项之间进行选择。
* `Swap Size` - 设置交换空间的大小。
...
.ZFS 池类型
image::bsdinstall-zfs-vdev_type.png%20是一个图片文件的名称。["请求虚拟设备类型的菜单。例如：条纹", mirror, raidz1]
...
一旦选择了“池类型”，将显示可用磁盘列表，并提示用户选择一个或多个磁盘来组成池。然后，将验证配置以确保选择了足够的磁盘。如果验证失败，请选择 btn:[< 更改选择 >] 返回到磁盘列表，或选择 btn:[< 返回 >] 更改“池类型”。

[[bsdinstall-zfs-disk_select]]
.磁盘选择
image::bsdinstall-zfs-disk_select.png%20是一个文件名，无法直接翻译。["菜单请求添加多少个磁盘到池中"]
...
为了避免意外擦除错误的磁盘，可以使用 btn:[- 磁盘信息] 菜单来检查每个磁盘，包括其分区表和其他各种信息，如设备型号和序列号（如果有）。
...
.磁盘加密密码
image::bsdinstall-zfs-geli_password.png%20是一个文件名，无法直接翻译。["要求输入密码以加密设备的菜单。"]
...
.扫描无线接入点
image::bsdinstall-configure-wireless-scan.png%20是一个图片文件的名称。["显示无线网络扫描的菜单。"]

无线网络通过服务集标识符（ SSID ）进行识别，每个网络都有一个短而独特的名称。扫描期间找到的 SSID 将列出，然后是该网络可用的加密类型的描述。如果所需的 SSID 在列表中未出现，请选择 btn:[重新扫描] 进行再次扫描。如果所需的网络仍未出现，请检查天线连接是否有问题，或尝试将计算机移动到接入点附近。每次更改后都要重新扫描。

[[bsdinstall-wireless-accesspoints]]
.选择无线网络
image::bsdinstall-configure-wireless-accesspoints.png%20是一个图片文件的名称。["显示可连接的不同无线网络的菜单。"]
...
.WPA2 设置
image::bsdinstall-configure-wireless-wpa2setup.png%20是一个图片文件的名称。["请求无线网络密码的菜单。"]
...
如果有 DHCP 服务器可用，请在下一个菜单中选择 btn:[是] 以自动配置网络接口。安装程序将会暂停一分钟左右，以便找到 DHCP 服务器并获取系统的地址信息。

[[bsdinstall-net-ipv4-dhcp]]
.选择 IPv4 DHCP 配置
image::bsdinstall-configure-network-interface-ipv4-dhcp.png%20是一个图像文件的名称。["菜单指示是否要为所选接口配置 DHCP 。"]
...
* `IP 地址 ` - 分配给此计算机的 IPv4 地址。该地址必须是唯一的，并且在本地网络上没有被其他设备使用。
* ` 子网掩码 ` - 网络的子网掩码。
* ` 默认路由器 ` - 网络的默认网关的 IP 地址。

下一个屏幕将询问是否应该为 IPv6 配置接口。如果 IPv6 可用且需要，请选择 btn:[是] 来选择它。

[[bsdinstall-net-ipv6]]
.选择 IPv6 网络
image::bsdinstall-configure-network-interface-ipv6.png%20是一个网络界面%20IPv6%20配置的截图。["菜单指示是否要为所选接口配置 IPv6 。"]

IPv6 还有两种配置方法。无状态地址自动配置（ SLAAC ）将自动从本地路由器请求正确的配置信息。有关更多信息，请参阅 http://tools.ietf.org/html/rfc4862[rfc4862] 。静态配置需要手动输入网络信息。

如果有 IPv6 路由器可用，请在下一个菜单中选择 btn:[是] 以自动配置网络接口。安装程序将会暂停一分钟左右，以查找路由器并获取系统的寻址信息。
...
.IPv6 静态配置
image::bsdinstall-configure-network-interface-ipv6-static.png%20是一个图片文件的名称。["菜单请求配置 IPv6 网络的数据。"]

* `IPv6 地址 ` - 分配给此计算机的 IPv6 地址。该地址必须是唯一的，并且在本地网络中没有被其他设备使用。
* ` 默认路由器 ` - 网络的默认网关的 IPv6 地址。

最后一个网络配置菜单用于配置域名系统（ DNS ）解析器，该解析器将主机名转换为网络地址，并且可以将网络地址转换为主机名。如果使用 DHCP 或 SLAAC 自动配置网络接口，则“解析器配置”值可能已经填写。否则，请在“搜索”字段中输入本地网络的域名。 `DNS #1` 和 `DNS #2` 是 DNS 服务器的 IPv4 和 / 或 IPv6 地址。至少需要一个 DNS 服务器。

[[bsdinstall-net-dns-config]]
.DNS 配置
image::bsdinstall-configure-network-ipv4-dns.png%20是一个图片文件的名称。["菜单请求数据以配置网络的 DNS 。"]
...
image::bsdinstall-netinstall-mirrorselect.png%20是一个图像文件的名称。["请求网络镜像的菜单。"]
...
.选择一个时区
image::bsdinstall-timezone-zone.png%20是一个图片文件的名称。["请求时区的菜单。"]

使用箭头键选择适当的时区，然后按下 kbd:[Enter] 键确认。

[[bsdinstall-timezone-confirmation]]
.确认时区
image::bsdinstall-timezone-confirm.png%20是一个图片文件，无法直接翻译为中文。["菜单请求确认所选时区。"]
...
使用箭头键选择适当的日期，然后按下 btn:[设置日期] 。否则，可以通过按下 btn:[跳过] 来跳过日期选择。

[[bsdinstall-timezone-time]]
.选择时间
image::bsdinstall-timezone-time.png%20是一个图片文件名，无法直接翻译。["请求系统时间的菜单。"]

使用箭头键选择适当的时间，然后按下 btn:[设置时间] 。否则，可以通过按下 btn:[跳过] 来跳过时间选择。
...
.选择启用附加服务
image::bsdinstall-config-services.png%20是一个图片文件的名称。["显示不同服务的菜单。"]
...
.选择加固安全选项
image::bsdinstall-hardening.png%20是一个图片文件的名称。["显示不同的加固安全选项的菜单。"]
...
.输入用户信息
image::bsdinstall-adduser2.png%20是一个图片文件名，无法翻译。["菜单请求新用户的不同信息。"]
...
.退出用户和组管理
image::bsdinstall-adduser3.png%20是一个图片文件名，无法直接翻译为中文。["菜单显示新用户的信息，并询问是否一切正确。"]
...
.最终配置
image::bsdinstall-finalconfiguration.png%20是一个文件名，无法直接翻译。["在完成安装之前显示不同选项的菜单。例如：添加用户", Time Zone, etc.]
...
bsdinstall 将提示进行任何在重启进入新系统之前需要完成的额外配置。选择 btn:[是] 退出到新系统中的 shell ，或选择 btn:[否] 继续进行安装的最后一步。

[[bsdinstall-final-main]]
.完成安装
image::bsdinstall-mainexit.png%20是一个文件名，无法直接翻译。["菜单显示安装已完成，并询问是否重新启动系统或访问 Live CD 。"]

如果需要进一步配置或特殊设置，请选择 btn:[Live CD] 以将安装介质引导到 Live CD 模式。

如果安装完成，请选择 btn:[重新启动] 来重新启动计算机并启动新的 FreeBSD 系统。不要忘记移除 FreeBSD 安装介质，否则计算机可能会再次从它启动。
...
请查看链接 https://www.FreeBSD.org/releases/[FreeBSD Release Information] 页面上列出的硬件说明，以确保硬件得到支持。
